...
void output_score(vector<vector<float> >& sim_matrix, ofstream& outfile){
    ...
    float opt = opt_score[sim_matrix.size()-1][sim_matrix[0].size()-1]/sim_matrix.size();
    outfile<<opt<<" ";
    avg_score += opt;
    path_count += 1;
...
//    subFolder.push_back("0");
    ...
    cout<<"_ ";
    for (int i=0;i<tree_names.size();i++){
        cout<<tree_names[i]<<" _ _ _ _ ";
    }
    cout<<endl;
    cout<<"Folder_Number ";
    for (int i=0;i<tree_names.size();i++){
        cout<<"Avg_Score_with_max_score Avg_Score_with_avg_score Max_Score Min_Score Total_Number_of_Path ";
    }
    cout<<endl;
