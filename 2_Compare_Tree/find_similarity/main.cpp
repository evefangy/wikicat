...
void output_similarity_matrix (vector<vector<string> >& path_m, vector<vector<string> >& path_n, ofstream& outfile){
    ...
    /* DEBUG 10/18 Treat last row differently as there's only one way to match it */
    // calculate the similarity score between the nodes containing the target article   
    status = calculate_node_similarity_score(path_m, path_n, curr_score, 0, 0);
    if (status == -1){
    	outfile<<"Fail"<<endl;
    	return;
    }
    ...
    for(int i=m-2;i>=0;i--){
        for(int j=n-1-index_m; j>=m-1-index_m; j--){
            // calculate_node_similarity_score(path_m[index_m], path_n[index_n], curr_score);
            
        	status = calculate_node_similarity_score(path_m, path_n, curr_score, index_m, index_n);
        	if (status == -1){
    			outfile<<"Fail"<<endl;
    			return;
    		}
